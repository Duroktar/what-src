...
describe('what-src common utilities', () => {
  describe('getIn', () => {
    it('should work for test inputs', () => {
      ...
      cases.forEach(({ value, expectedResult }) => {
        expect(U.getIn(value[0] as any, value[1])).toEqual(expectedResult)
      })
    })
    it('should fail on input of the wrong type\n' +
      '\tbut not if passed a null or undefined object',
    () => {
      expect(() => U.getIn(undefined as any, {})).toThrow()
      expect(() => U.getIn(null as any, {})).toThrow()
      expect(() => U.getIn(0 as any, {})).toThrow()
      expect(() => U.getIn({} as any, {})).toThrow()
      expect(() => U.getIn('', undefined as any)).not.toThrow()
      expect(() => U.getIn('', null as any)).not.toThrow()
    })
  })
  ...
  describe('withOnOff', () => {
    it('should call the toggle with true before calling the\n' +
        '\tcb and then finally calling the toggle again with false',
    async() => {
      const states: Array<boolean | number> = []
      const toggle = (n: boolean) => states.push(n)
  ...
  describe('withHooks', () => {
    ...
    () => {
      const states: any[] = []
      const callback = (n: any) => states.push(n)
  ...
  describe('asynchronous retry function', () => {
    ...
    const getTest = async(n: number, d: number = 1) => {
